...
def plot_multipole_clusters(model, img_path, checkpoint_path, stage_idx=2, block_idx=0):
    """
    model: instantiated model object
    img_path: infrared img path
    checkpoint_path: .pth file path
    ...
    # 3. inference (save cluster_ids)
    with torch.no_grad():
        # model.forward deal with padding and params
        _ = model(img_tensor)

    # 4. extract cluster data
    ...
    try:
        ...
        print("cluster_ids not found, check whether saved and correct the index")
        return

    # 5. reshape to 2D and draw
    # h, w should equal to feature image size input to ASSM
    res_map = cluster_id_map[0].view(h_orig, w_orig).numpy()
    ...
    for i in range(6):
        for j in range(4):
            plot_multipole_clusters(
                model=my_model,
                # img_path='datasets/hm_tir/val/noise/01200.png',       # change to your test image
                img_path='multi_noise_addition/out_noisy_image/00000.png',       # change to your test image
                checkpoint_path='experiments/HM-TIR-MULTIPOLE/wavelet-focalnet_multipole-mamba_K32_3e-4_67epoch_6.9M_0108/models/net_g_20000.pth',     # change to your weight
                stage_idx=i,  # check middle or deep layer
                block_idx=j
            )
